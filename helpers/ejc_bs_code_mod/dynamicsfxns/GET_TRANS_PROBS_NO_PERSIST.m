...
transProbEndGivenStart = zeros(size(numTransitions)); %make NaN

inclInds = unique(subjInd); 
for N = 1:length(unique(inclInds))
    ind = unique(subjInd, 'stable'); ind = ind(N);
    subjMask = subjInd == ind;
