...
dem_var = {'src_subject_id','eventname','race_ethnicity','demo_comb_income_v2',...
    ...
    'demo_prnt_ed_v2', 'demo_prtnr_ed_v2','demo_prim','demo_adopt_agex_v2'}; 
dem_var_names = {'subjectkey','eventname','race','comb_income',...
    'parent_income', 'partner_income', 'parentEd', 'partnerEd', 'prim_id', 'age_adopted'};
...
for s = 1:height(fhxssp)
    ...
    elseif fhxssp.FHSUD(s) == "FH+/-"
        if fhxssp.grandparentAlc(s) == 1
            typeSUD{s} = 'AUD';
        elseif fhxssp.grandparentDrug(s) == 1
            typeSUD{s} = 'DUD';
        end
    else 
         typeSUD{s} = 'NaN';
...
fhxssp.typeSUD = categorical(cellstr(typeSUD)));
...
%% Parental history of mental illness (binary):
...
dhxs_var = {'src_subject_id','devhx_9_tobacco','devhx_9_alcohol' 'devhx_9_marijuana',...
    'devhx_9_her_morph', 'devhx_9_oxycont','devhx_9_coc_crack','devhx_9_other_drugs','devhx_9_other1_name_2',...
...
dhxs_var_names = {'subjectkey','pregtobacco','pregalcohol','pregmarijuana', ...
    'pregmorph','pregoxy','pregcrack', 'pregother',...
...
dhxs.missingPregSub = isnan(dhxs.pregtobacco)|isnan(dhxs.pregalcohol)|...
    isnan(dhxs.pregmarijuana)|isnan(dhxs.pregmorph)| isnan(dhxs.pregcrack)|...
    isnan(dhxs.pregoxy)|isnan(dhxs.pregother);

dhxs.subDuringPreg = (dhxs.pregtobacco) == 1 | (dhxs.pregalcohol == 1) | ...
    (dhxs.pregmarijuana) == 1 | (dhxs.pregmorph) == 1 |(dhxs.pregcrack) == 1 | ...
...
% 
% pub_var = {'src_subject_id', 'pds_y_ss_male_category','pds_y_ss_female_category'};
% pub_var_names = {'subjectkey', 'puberty_male_child', 'puberty_female_child'}; 
% pub_child = readtable([basedir '/physical-health/ph_y_pds.csv']);
% pub_child = extractInstrument(pub_child, pub_var, pub_var_names,event,subjs);
% pub_child.puberty_combined_child = pub_child.puberty_male_child; % Initialize with male scores
% nan_male = isnan(pub_child.puberty_male_child); % Find rows where male scores are NaN
% pub_child.puberty_combined_child(nan_male) = pub_child.puberty_female_child(nan_male);
...
% Combine parent puberty scores
pub.puberty_combined = pub.puberty_male; % Initialize with male scores
nan_male = isnan(pub.puberty_male); % Find rows where male scores are NaN
pub.puberty_combined(nan_male) = pub.puberty_female(nan_male); % Fill with female scores if male scores are NaN

% Load children's puberty scores
pub_var_child = {'src_subject_id', 'pds_y_ss_male_category','pds_y_ss_female_category'};
pub_var_names_child = {'subjectkey', 'puberty_male_child', 'puberty_female_child'}; 
pub_child = readtable([basedir '/physical-health/ph_y_pds.csv']);
pub_child = extractInstrument(pub_child, pub_var_child, pub_var_names_child, event, subjs);

% Combine child puberty scores
pub_child.puberty_combined_child = pub_child.puberty_male_child; % Initialize with male scores
nan_male_child = isnan(pub_child.puberty_male_child); % Find rows where male scores are NaN
pub_child.puberty_combined_child(nan_male_child) = pub_child.puberty_female_child(nan_male_child);

% Merge child and parent tables based on subjectkey
pub_combined = outerjoin(pub, pub_child, 'Keys', 'subjectkey', 'MergeKeys', true);

% Final combination: Use parent data where available, otherwise use child data
pub_combined.puberty_combined = pub_combined.puberty_combined; % Start with parent data
nan_parent = isnan(pub_combined.puberty_combined); % Identify rows where parent data is missing
pub_combined.puberty_combined(nan_parent) = pub_combined.puberty_combined_child(nan_parent); % Use child data if parent data missing
...
tabs = {site, mri, sex, dem,ehis,res_coi,res_adi,nsc, pm_y, pm_p, fes, ...
    ...
    dhxs,st_p,su,pub_combined,horm,ksads_sud,upps,sst}; 
...
savedir = '/Users/louisaschilling/Desktop/FINAL CODE PAPER/Data'; cd(savedir);
