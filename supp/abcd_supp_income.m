% Supplemental figure: by income group 
clear all; close all; 
...
for i = 1:3
    ...
    anovaVarNames = {'Sex','Age','FHSUD','FD','Race','In Utero Substances','Parental History Mental Health'};
    interaction_term_sets = {{'Sex', 'FHSUD'}};
    contVar = [2,4];
    anovaVars = {subjectInfo.sex(inds),subjectInfo.age(inds),subjectInfo.FHSUD(inds)== 'FH+',...
        subjectInfo.FD_mean(inds),subjectInfo.race(inds), ...
        subjectInfo.subDuringPreg(inds), subjectInfo.parentMH(inds)};
    [all_results] = run_ANOVA(globalTE(inds),anovaVars,contVar,anovaVarNames,interaction_term_sets,'off');
    disp(i); disp(all_results); 
...
%% network boxplot
networks = {'vis','som','dat','vat','lim','fpn','dmn','sub','cer'}; 
clear g; f= figure('Position', [100,100,1200,1000]);
...
for i = 1:3
    ...
    g(2,i).geom_abline('intercept', 0, 'slope', 0, 'style', 'k--');
    g(2,i).axe_property('TickDir','out', 'Ygrid','on','GridColor',[0.5 0.5 0.5],'YLim', [-0.1, 11.5]);
...
g.draw();
exportgraphics(f, [figDir '/suppFig_byincome_globalnetworkTE_k' num2str(numClusters) '.png']);

%%
clear g; 
f= figure('Position',[100,100,1000,500]);
g(1,1) = gramm('x', subjectInfo.sexFHSUD,'subset', ...
    (subjectInfo.FHSUD == 'FH+' | subjectInfo.FHSUD == 'FH-'));
g(1,1).stat_bin();
g(1,1).facet_grid([], subjectInfo.income_cat);
g(1,1).set_text_options('base_size', 18,'font','calibri','title_scaling', 1);
g(1,1).set_names('x', '','y', 'count', 'color', 'Significance','column', 'Income');
g(1,1).set_color_options('map','brewer_dark');
g.draw();
exportgraphics(f, [figDir '/suppFig_byIncome_demo_.png']);
